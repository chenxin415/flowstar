...
//	intervalNumPrecision = 53;
	...
	// x' = 1 + x*(x*y - 4), y' = x*(3 - x*y), t' = 1
	ODE<Real> ode({"1 + x*(x*y - 4)", "x*(3 - x*y)", "1"}, vars);
	...
	Interval init_x(1.48, 1.52), init_y(2.98, 3.02);
	...
	plot_setting.plot_2D_octagon_GNUPLOT("./", "brusselator_t_x", result.tmv_flowpipes, setting); // GNUPLOT file

	plot_setting.setOutputDims("t", "y");
	plot_setting.plot_2D_octagon_GNUPLOT("./", "brusselator_t_y", result.tmv_flowpipes, setting); // GNUPLOT file
